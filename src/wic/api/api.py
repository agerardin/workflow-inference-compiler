...
class Workflow:
    ...
    def compile(
        ...
        print(proc.stdout)
        if out_dir:
            workflow_filename = f"{self.name}.cwl"
            workflow_inputs_filename = f"{self.name}_inputs.yml"
            workflow_path = _path(wic_dir).joinpath(
                "autogenerated", workflow_filename
            )
            inputs_path = _path(wic_dir).joinpath(
                "autogenerated", workflow_inputs_filename
            )
            workflow_copy = _path(out_dir).joinpath(workflow_filename)
            shutil.copy(workflow_path, workflow_copy)
            workflow_inputs_copy = _path(out_dir).joinpath(workflow_inputs_filename)
            shutil.copy(workflow_path, workflow_inputs_copy)
            
            print(f"Successfully compiled to {str(workflow_copy)} and {str(workflow_inputs_copy)}")
